...
import (
	...
	apis "github.com/arubacloud/provider-arubacloud/apis"
	"github.com/arubacloud/provider-arubacloud/config"
	"github.com/arubacloud/provider-arubacloud/internal/clients"
	controllers "github.com/arubacloud/provider-arubacloud/internal/controller"
	"github.com/arubacloud/provider-arubacloud/internal/features"
	"github.com/arubacloud/provider-arubacloud/internal/version"
)
...
func main() {
	var (
		app                     = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for ArubaCloud").DefaultEnvars()
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-arubacloud",
		Cache: cache.Options{
	...
	kingpin.FatalIfError(apis.AddToScheme(mgr.GetScheme()), "Cannot add ArubaCloud APIs to scheme")
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(controllers.SetupGated(mgr, opts), "Cannot setup ArubaCloud controllers")
	} else {
		log.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(controllers.Setup(mgr, opts), "Cannot setup ArubaCloud controllers")
	}
